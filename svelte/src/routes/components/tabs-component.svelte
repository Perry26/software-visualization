<script lang="ts">
	...
	export let sidePanelTab: SidePanelTab;
	...
	{#each tabs as tab, i}
		<li class="w-full focus-within:z-10">
			<button
                ...
                ${i === tabs.length - 1 ? lastClasses : middleClasses}
                ${i === 0 ? firstClass : ''}
                ${
									//@ts-ignore
									sidePanelTab === SidePanelTab[tab] ? selectedClass : notSelectedClass
								}
                `}
				aria-current="page">{tab}</button
			>
		</li>
	{/each}
</ul>
